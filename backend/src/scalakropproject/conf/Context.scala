...
object Context {
  ...
  val development: Resource[IO, Context] =
    Sqlite.create("./scalakropproject-development.sqlite3").map(xa => Context(xa))

  // The production mode context
  val production: Resource[IO, Context] =
    Sqlite.create("./scalakropproject-production.sqlite3").map(xa => Context(xa))
